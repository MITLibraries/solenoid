...
class EmailCreate(ConditionalLoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return HttpResponseRedirect(reverse("emails:evaluate", args=(first_pk,)))
        except IndexError:
...
class EmailEvaluate(ConditionalLoginRequiredMixin, UpdateView):
    ...
    def _check_record_count(self):
        available_records = Record.objects.filter(author=self.object.author).exclude(
        ...
        if available_records.count() > self.object.record_set.count():
            messages.error(
                ...
                "to <a href='{url}'>add them?</a>".format(
    ...
    def _handle_send(self):
        ...
        messages.success(
            self.request,
            "Email message updated and sent. "
    ...
    def _update_session(self):
        if "email_pks" not in self.request.session or not len(
            self.request.session["email_pks"]
        ):
            return None
    ...
    def get_context_data(self, **kwargs):
        ...
        context["breadcrumbs"] = [
            ...
            {"url": reverse("emails:list_pending"), "text": "view pending emails"},
            {"url": "#", "text": "send email"},
        ]
...
class EmailSend(ConditionalLoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for pk in pk_list:
            ...
            sent = EmailMessage.objects.get(pk=pk).send(self.request.user.username)
            statuses.append(sent)

        if False in statuses:
            logger.warning(
                "Could not send all emails. {bad} of {all} were "
                "not sent.".format(bad=statuses.count(False), all=len(pk_list))
            )
            messages.warning(
                request,
...
class EmailListPending(ConditionalLoginRequiredMixin, ListView):
    def get_queryset(self):
        qs = EmailMessage.objects.filter(date_sent__isnull=True).prefetch_related(
            "author"
        )
        return qs
...
class EmailRebuild(ConditionalLoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        records = Record.objects.filter(author=email.author).exclude(
            email__date_sent__isnull=False
        )
        ...
        return HttpResponseRedirect(reverse("emails:evaluate", args=(email.pk,)))
